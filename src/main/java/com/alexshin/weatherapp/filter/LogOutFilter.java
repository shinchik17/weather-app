...
public class LogOutFilter extends BaseFilter {
    ...
    protected void doFilter(HttpServletRequest req, HttpServletResponse resp, FilterChain chain) throws IOException, ServletException {
        ...
        logger.info("LogOutFilter -> Enter doFilter(), servletPath: %s, method: %s".formatted(servletPath, method));

        Optional<String> optSessionId = CookieUtil.extractSessionCookie(req);

        if (optSessionId.isEmpty()) {
            redirectToRootContext(resp);
            logger.info("LogOutFilter -> Session is empty, redirect to the rootContext");
            return;
        }
        ...
        logger.info("LogOutFilter -> Process to next filter");
